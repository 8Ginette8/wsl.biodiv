# ###########################################################################
# Second order functions for model fitting frame work on which fiz and 
# peval depend.
#
# $Date: 2015-08-20 V1
# $Date: 2018-03-14 V2
...
### =========================================================================
### generate Predictors for GAM and GLM (fiz)
### =========================================================================

# generate predictors for gam models
predictors<-function(dataf,response,type,mod){
  
  # Variables used
  # assuming all variables are numeric and relevant
  vars<-colnames(dataf)[-which(colnames(dataf)%in%response)]
  ...
  if(interact && mod=="glm"){
      ...
      for(i in 2){
        ...
        in.te<-append(in.te,pst)
      }
      
      int.part=paste(in.te,collapse="+")
  }
  ...
  if(mod=="gam"){
    ...
    sm.vars<-paste("s(",vars,kapart,")",sep="")
    add.part<-paste(sm.vars,collapse="+")
  }
  ...
  if(mod=="glm"){
    ...
    if(type=="simple"){
      add.part<-paste(paste0("poly(",vars,",2)"),collapse="+")
    
    } else if(type=="complex"){
      add.part<-paste(paste0("poly(",vars,",4)"),collapse="+")
    
    }
  }
  
  ### ------------------------
  ### Combine interactive and additive parts
  ### ------------------------ 

  form.pred=ifelse(exists("int.part"),paste(add.part,int.part,sep="+"),add.part)
  return(form.pred)
}
