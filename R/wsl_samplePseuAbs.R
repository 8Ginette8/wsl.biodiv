...
#' @param target.group_dir Directory where xy files of target group taxa are
...
wsl.samplePseuAbs<-function(n=10000,
  ...
  if(type=="geographic"){
    ...
    abs=sampleRegular(rst,round(length(pres)*geodist_fact),sp=TRUE)
    ...
    for(i in 1:geo_nrep){
      ...
      smp=sample(1:length(prb),size=round(n*(1-add.strat+.1)/geo_nrep),prob=prb,replace=TRUE)
    ...
    pts=sample(1:length(vls),n*(1-add.strat)*1.1,prob=vls,replace=TRUE)
